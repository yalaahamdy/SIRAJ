...
### مصفوفة الاختبارات الأساسية (Core Test Matrix - 187+ Tests):

| ملف الاختبار | المجال والوحدة | سيناريوهات التحقق والفحص |
| :--- | :--- | :--- |
| **`test/modules/quran/quran_offline_audio_service_test.dart`** | خدمة التلاوة واستيراد الـ ZIP | • فك ضغط حزم ZIP بنظام التدفق بدون تراكم في الذاكرة<br>• فحص سلامة ملفات الآيات من `001001.mp3` إلى `114006.mp3`<br>• التعامل مع ملفات ZIP التالفة أو الفارغة وإرجاع رسائل خطأ واضحة |
| **`test/modules/quran/quran_recitation_matcher_test.dart`** | محرك التسميع والتعرف الصوتي | • مطابقة النص الصوتي اللحظي عبر نافذة الكلمات الثلاث المتزامنة<br>• تلوين الكلمات الخاطئة بالأحمر واستمرار التسميع بسلاسة<br>• استقرار جلسة الميكروفون ومنع التوقف التلقائي أثناء الصمت المؤقت |
| **`test/modules/quran/quran_responsive_reader_test.dart`** | شاشة وقارئ المصحف الشريف | • انعدام تام لأخطاء الـ Overflow على الشاشات من 320px إلى الشاشات اللوحية<br>• تجاوب شريط العنوان المقاوم للاقتطاع (Zero Header Clipping)<br>• التصفح الأفقي المتصل والانتقال التلقائي بين السور |
| **`test/modules/prayer/exhaustive_boundaries_test.dart`** | محرك المواقيت الفلكية والقبلة | • تدقيق النقاط الحدية لـ 15 نقطة زمنية عبر دورة اليوم والليل<br>• اختبار المناطق القطبية وخطوط العرض العليا وطرق الحساب الخمس الكبرى<br>• فحص مثلثات القبلة الكروية في مكة المكرمة والمدن العالمية |
| **`test/modules/zakat/m04_professional_zakat_upgrade_test.dart`** | محرك وحاسبة الزكاة الذكية | • التحقق من نصاب الذهب والفضة بمختلف العملات الإقليمية والعالمية<br>• تدقيق شروط الحول والنصاب على 8 فئات أصول مالية واستثمارية |
| **`test/modules/knowledge/knowledge_production_data_test.dart`** | موسوعة الحديث والفقه والسيرة | • فحص وتوثيق صحاح الأحاديث (البخاري، مسلم، الأربعون النووية)<br>• سلامة متون وأسانيد الفقه الإسلامي والخط الزمني للسيرة النبوية |
...
## 📜 3. مسارات الأتمتة السحابية المستمرة (GitHub Actions CI/CD Workflows)

يعتمد المستودع مسارين رسميين للأتمتة المستمرة في مجلد `.github/workflows/`:

### أ. مسار الفحص والتحليل المستمر (`.github/workflows/ci.yml`):
- **الأحداث المشغلة (Triggers)**: كافة عمليات الـ Push والـ Pull Requests على فرعي `main` و `master`.
- **خطوات التحقق الآلية**:
  1. جلب الكود وضبط بيئة Java 17 و Flutter (`channel: 'stable'`).
  2. تثبيت الحزم عبر `flutter pub get` مع التحقق من التوافقية العريضة لـ SDK (`sdk: '>=3.5.0 <4.0.0'`).
  3. الفحص الساكن الصارم عبر `flutter analyze lib/`.
  4. تشغيل كامل مصفوفة الاختبارات الآلية (187 اختباراً) والتأكد من اجتيازها بنسبة 100%.

### ب. مسار بناء ونشر الإصدارات والـ APK آلياً (`.github/workflows/release.yml`):
- **الأحداث المشغلة**: دفع أي وسم إصدار جديد مطابق للنمط `v*` أو `V*` (مثل `v0.1.0` و `V0.1.0`).
- **خطوات البناء والنشر**:
  1. بناء حزمة أندرويد موقعة ومصغرة عبر `flutter build apk --release`.
  2. رفع الحزمة كمرفق بناء (Build Artifact) في مسار التشغيل.
  3. إنشاء ونشر صفحة الإصدار الرسمية (GitHub Release) تلقائياً عبر `softprops/action-gh-release@v2` مع إرفاق ملف `siraj-v0.1.0.apk` وملاحظات الـ `CHANGELOG.md`.
