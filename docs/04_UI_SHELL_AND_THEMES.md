...
4. **شريط العنوان المرن المقاوم للاقتطاع (Zero Header Clipping Architecture)**:
   - ضبط `centerTitle: false` في شريط التطبيق (`AppBar`) لمنح العنوان كامل المساحة الأفقية المتاحة بين زر الرجوع وأزرار الإجراءات.
   - تغليف نص السورة داخل `FittedBox(fit: BoxFit.scaleDown, alignment: AlignmentDirectional.centerStart)` مع `maxLines: 1` ليتم مواءمة الحجم تلقائياً عند السور ذات الأسماء الطويلة (مثل "سورة آل عمران").
   - تقليص الحشو العرضي للأزرار الأربعة عبر `VisualDensity.compact` لتوفير أكثر من 50 بكسل إضافية للعنوان.
5. **عرض المصحف المتصل وتناسق ترويسة الصفحة (Edge-to-Edge Mushaf Page View)**:
   - إلغاء الهوامش والإطارات الثقيلة الزائدة ليغطي المصحف كامل مساحة الشاشة بشكل متناسق مع الحفاظ على هوامش الأمان.
   - حماية اسم السورة ورقم الجزء في الترويسة الزخرفية العلوية بواسطة `Flexible` و `FittedBox`، مما يمنع حدوث أي تجاوز أو تشوه بصري في الأجهزة ذات الشاشات الضيقة.
